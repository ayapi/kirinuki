...
- **Findings**:
  ...
  - **制約**: trigramは3文字未満のクエリではマッチしない。日本語では2文字の意味ある単語が多い（演出、企画、音楽、漫画、映画等）ため、短いクエリには`subtitle_lines`テーブルへの`LIKE`フォールバックが必要
  - ICUトークナイザーはPython標準sqlite3では利用不可（コンパイルフラグが必要）
  - trigramはインデックスサイズが大きくなるが、10万セグメント規模なら問題なし
- **Implications**: FTS5 + trigramで外部依存なしに日本語キーワード検索を実現。ただし3文字未満のクエリにはLIKEフォールバックで対応する
