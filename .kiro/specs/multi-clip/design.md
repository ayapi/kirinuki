...
sequenceDiagram
    ...
    CLI->>CLI: build output paths with numbering
...
|-----------|--------------|--------|--------------|------------------|-----------|
| ClipCmd | CLI | コマンド引数パース・進捗表示・結果出力 | 1.1-1.4, 4.2, 5.1, 5.3 | ClipService (P0), AppConfig (P0) | — |
| ClipService | Core | 複数範囲の切り出しオーケストレーション | 2.1-2.3, 4.3, 5.2, 5.4 | YtdlpClient (P0) | Service |
| ClipUtils | Core | 時間範囲パース、ファイル名生成 | 1.2, 1.3, 3.1-3.3 | — | — |
| YtdlpClient | Infra | yt-dlp `download_ranges` による部分DL | 2.1 | yt-dlp (P0) | Service |
| MultiClipRequest | Models | 複数範囲リクエストのバリデーション | 2.3 | — | — |
...
| Requirements | 1.1, 1.2, 1.3, 1.4, 4.2, 5.1, 5.3 |
...
- 出力パスの組み立て: `output_dir / build_numbered_filename(filename, index, total)`
...
| Field | Detail |
|-------|--------|
| Intent | 複数範囲の切り出しオーケストレーション |
| Requirements | 2.1, 2.2, 2.3, 4.3, 5.2, 5.4 |
...
- Postconditions: 成功した範囲の出力ファイルが `request.output_dir` に存在する
...
- `MultiClipRequest` バリデーション: 正常系、不正範囲、空リスト
