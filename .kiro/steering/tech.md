...
### 出力フォーマット要件
Vrewがサポートする形式に合わせる: **コーデック h264/mp3、コンテナ mp4**

### 音ズレの原因と対策
- yt-dlpの`download_section`（範囲DL）は内部で`-c copy`（ストリームコピー）でカットするため、キーフレーム境界のずれから**edit list**が生成される
- edit listを正しく解釈しないプレイヤー（Vrew等）では音ズレが発生する
- **対策**: `download_section`後にffmpegで映像・音声を再エンコード（`-c:v libx264 -c:a libmp3lame`）し、edit listを根本的に排除する
- edit listの単純な削除は逆効果（映像がキーフレームから、音声が指定点から始まり、むしろズレが悪化する）

### 処理フロー
`download_section`（高速部分DL） → `ffmpeg reencode`（h264+mp3再エンコード） → Vrew互換mp4出力
